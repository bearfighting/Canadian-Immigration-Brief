# 加拿大移民中文信息网站：技术设计

> 状态：工作稿 v0.4
> 更新日期：2026-09-05

## 1. 技术目标

- 以静态内容为主，低成本、快速、可维护；
- 正式内容通过 Git 保存与审核；
- 首版不依赖数据库；
- 自动化只能生成草稿，不能绕过人工审核；
- 数据结构为未来数据库迁移保留清晰边界。

当前实施分为两个阶段：阶段一负责网站产品和技术能力；阶段二负责内容管理与发布运营。内容文件和审核字段属于网站需要支持的领域边界，但内容生产节奏、staging 发布和持续运营不应反向耦合页面组件或阻塞阶段一开发。

## 2. 推荐技术栈

- Next.js App Router + TypeScript；
- Tailwind CSS 4；
- shadcn/ui 作为基础组件，不直接决定视觉风格；
- 普通 Markdown + YAML front matter；
- Zod 校验内容与数据；
- pnpm 管理依赖；
- Vitest + Playwright 测试；
- 构建时静态搜索索引；
- Vercel 或同类静态/混合部署；
- GitHub Actions 运行检查。

站点绝对 URL 由 `NEXT_PUBLIC_SITE_URL` 优先覆盖；未配置时在 Vercel 使用 `VERCEL_PROJECT_PRODUCTION_URL` 或 `VERCEL_URL`，本地开发和无部署环境回退到 `http://localhost:3000`。生产环境使用的非本地 URL 必须为 HTTPS。

首版优先普通 Markdown，而不是允许任意 React 表达式的 MDX。确有交互图表需求时，对少量可信页面开放 MDX。具体依赖版本和库选择在实现前锁定，并写入 package.json 与 ADR。

M1 使用 App Router 静态生成；CSS variables 是颜色和圆角的单一来源，Tailwind 负责布局与响应式组合。重复的交互样式通过基础 UI 变体复用，页面特有的布局组合保留在组件中。中文字体采用系统优先字体栈，不加载远程字体。

公开页面、RSS、sitemap 和搜索索引统一消费经过状态过滤的公开内容集合；原始 Content 不直接暴露给输出层。

首页快速发现使用原生 GET 表单和现有内容查询参数，栏目、地区和项目入口复用受控词表；政策状态保留在内容浏览器的高级筛选中，不作为首页主要发现入口。首页不复制过滤逻辑，也不引入第三方搜索服务、自动聚类或用户行为统计。

当前网站开发以 News Infrastructure 为重点。新闻继续使用现有 camelCase 内容模型和统一公开投影，不直接改用外部计划中的 snake_case 字段；`officialSources` 与 `supplementarySources` 继续区分官方来源和补充来源。新闻日期字段逐步支持 `publishedAt`、`announcedAt`、`eventAt`、`effectiveAt`、`updatedAt` 和 `lastVerifiedAt`，新增字段保持 optional，以兼容现有 Markdown 内容。

新闻列表和详情页优先静态生成，主要文章内容必须在构建生成的 HTML 中可读；客户端 JavaScript 只负责筛选、分页和其他增强交互。`policyStatus` 保留为编辑、审核、检索和自动化工作流字段，新闻默认不把它作为视觉状态标签；只有在未生效、暂停或已被替代等会影响读者判断的情况下，才生成读者提示。日期相关提示以内容状态和日期字段为准，并在内容更新后通过重新构建刷新，不依赖访问时的客户端时钟。新闻 Fact Box、来源卡片、相关项目入口和 RSS/sitemap/JSON-LD 统一消费公开内容集合。文章级更新/更正历史、AI News Desk、自动采集和自动发布属于阶段二内容管理与发布运营，不进入页面基础设施。

## 3. 建议代码结构

```text
app/
├── page.tsx
├── news/
├── rss.xml/
├── robots.ts
├── sitemap.ts
└── globals.css

components/
├── content/
├── policy/
├── navigation/
└── charts/

content/
data/
lib/
├── content/
├── search/
├── seo/
└── validation/
```

内容详情页由 `components/content/content-detail-layout.tsx` 统一编排标题、正文、关键信息、分享、来源和相关内容，来源与相关内容区块由 `content-detail-sections.tsx` 复用；新闻与非新闻页面只负责传入类型差异。列表查询参数解析和生成集中在 `lib/content/query.ts`，内容过滤和分页保留在 `lib/content/browser.ts`。领域状态类型位于 `lib/content/types.ts`，供校验层和界面层共同使用，避免界面直接依赖 Schema 定义。

## 4. 内容构建流程

```text
Markdown/JSON
  -> 解析 front matter
  -> Zod 校验
  -> 解析 Markdown AST
  -> 建立内容关系
  -> 生成静态页面
  -> 生成搜索索引、RSS、sitemap 和 JSON-LD
```

构建必须在以下情况失败：非法标签、重复 ID/slug、发布内容缺少官方来源、未批准内容被标记为发布、无法解析的项目引用、失效的版本关系或不合理日期。

## 5. 渲染与安全

- Markdown HTML 默认转义；
- 禁止不受信任的内联脚本和任意组件；
- 外部链接增加合理的安全属性；
- 官方来源 URL 仅允许 HTTPS，并通过维护中的官方域名白名单检查；
- 官方来源和补充来源使用不同的 Schema，补充来源不要求官方域名；
- 自动生成文件只进入草稿目录或独立分支；
- 生产构建排除 `publicationStatus: draft` 和 `review.status != approved`。

## 6. 搜索

首版使用构建时静态索引，不需要数据库全文搜索。索引正文、标题、description、官方名称、缩写、中文别名、项目、省份、受众和状态。

搜索结果支持：

- 内容类型过滤；
- 省份和主题过滤；
- 当前指南与历史新闻区分；
- 常用中文简称和错别名映射；
- 无结果关键词统计（在符合隐私策略的前提下）。

首版先选择一种构建时搜索实现并封装接口；搜索实现不应渗透到内容模型和页面组件中。

## 7. SEO

- 稳定、无日期依赖的公开 slug；
- canonical URL；
- sitemap 与新闻/栏目 RSS；
- `NewsArticle`、`Article` 和 `BreadcrumbList` JSON-LD；
- `datePublished`、`dateModified` 和作者信息；
- 当前指南与历史新闻使用不同的结构化数据类型和视觉标识；
- 基础 Open Graph metadata；分享图延期至后续视觉资源准备完成后；
- 详情页分享使用本地组件、平台分享链接和 Web Share API，不加载社交平台 SDK，不记录分享统计；
- 标签页达到最低内容量后才允许索引；
- 搜索、组合筛选、草稿和薄内容页设置 `noindex`。

## 8. 性能

- 页面以静态生成为主；
- 限制客户端 JavaScript；
- 不为新闻强制加载大图；
- 字体子集化并合理回退；
- 图表按需加载；
- 目标是主要内容无需等待客户端水合即可阅读。

## 9. 自动化编辑流程

```text
定时来源扫描
  -> 候选新闻与去重
  -> 字段提取和中文初稿
  -> draft 文件或 Pull Request
  -> 人工核验与修改
  -> review.status=approved
  -> 合并并发布
```

早期可以手工将定期选题简报转成文章。待来源和流程稳定后，再开发采集器及网页差异检测。

## 10. 测试

### 单元测试

- Schema 与受控词表；
- 日期和状态逻辑；
- 内容关系解析；
- 术语别名；
- SEO metadata。

### 构建测试

- 所有已发布页面可生成；
- 内部链接无断链；
- sitemap、RSS 和 JSON-LD 合法；
- 草稿不可公开访问。
- 生产构建不包含未批准内容和测试数据。

### UI 测试

- 首页、新闻页、项目页和搜索页关键视口；
- 状态标签不仅依靠颜色；
- 手机端表格可用；
- 键盘导航和焦点状态。

## 11. 运行日志与隐私

首版不接入产品分析工具，不主动收集页面访问、入口、搜索词、无结果搜索、项目页回访或官方来源点击等产品分析数据。仅保留部署平台和服务器运行所必需的技术日志，并在隐私政策中说明日志类型、保留期限和第三方服务。未来重新引入分析工具时，必须新增 ADR。

## 12. 备份和内容完整性

- Git 保存文章、数据、术语和重定向；
- 定期验证外部链接；
- 图片和图表资产使用稳定路径；
- slug 变化必须增加重定向；
- 数据文件带 `schemaVersion` 和 `updatedAt`；
- 重要官方页面未来可保存摘要、哈希和访问日期，避免复制完整版权内容。
- 发布前应有 staging 预览环境；生产发布失败时可以回滚到上一个 Git 版本；
- CI 至少运行 Schema 校验、链接检查、构建检查和关键页面 smoke test。

## 13. 数据库引入边界

数据库优先用于：候选新闻、网页快照、抓取状态、订阅偏好、通知历史、账户和复杂数据筛选。正式出版文章可以继续保留在 Markdown。

推荐演进：

1. 静态 Markdown + JSON；
2. Markdown 出版 + Postgres 编辑工作流；
3. 在真实需求出现后加入账户、个性化与专业数据服务。

## 14. 上线检查

- [ ] 内容 Schema 全部通过；
- [ ] 核心页面移动端和桌面端验收；
- [ ] 404、重定向和失效内容提示可用；
- [ ] sitemap、RSS、robots 和 canonical 正确；
- [ ] 编辑标准、免责声明和隐私政策上线；
- [ ] 监控构建失败与运行错误；
- [ ] 准备回滚方式；
- [ ] 至少完成一轮真实新闻发布演练。

## 15. 已决定、延期与待决事项

- Next.js 具体版本与内容解析库；
- 静态搜索实现选择；
- 已延期：分析工具；
- 已延期：邮件订阅服务；
- 图片存储策略；
- 是否建立独立 staging 域名；
- 自动化初稿采用文件、Issue 还是 Pull Request。

技术实施应按 milestones.md 推进；Milestone 1 的基础校验和 staging 完成前，不进入批量内容生产。
